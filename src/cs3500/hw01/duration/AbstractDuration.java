...
abstract class AbstractDuration implements Duration {
  ...
  public String format(String template) {
    try {
      ...
      throw new IllegalArgumentException("Tried to pass in a null template");
    }
    ...
    while (template.length() > 0) {
      ...
      if (String.valueOf(template.charAt(0)).equals("%")) {
        if (template.length() == 1) {
          throw new IllegalArgumentException(
                  "Ends with a hanging '%' -- template is illformed");
        }
        String toAdd;
        String second;

        second = String.valueOf(template.charAt(1));
        switch (second) {
          case "%":
            toAdd = "%";
            break;
          case "t":
            toAdd = String.format("%d", this.inSeconds());
            break;
          case "h":
            toAdd = String.valueOf(this.hoursOf(this.inSeconds()));
            break;
          case "H":
            toAdd = String.format("%02d", this.hoursOf(this.inSeconds()));
            break;
          case "m":
            toAdd = String.valueOf(this.minutesOf(this.inSeconds()));
            break;
          case "M":
            toAdd = String.format("%02d", this.minutesOf(this.inSeconds()));
            break;
          case "s":
            toAdd = String.valueOf(this.secondsOf(this.inSeconds()));
            break;
          case "S":
            toAdd = String.format("%02d", this.secondsOf(this.inSeconds()));
            break;
          default:
            throw new IllegalArgumentException(
                    "Invalid character follows '%' -- template is illformed");
        }
        output = output.concat(toAdd);
        template = template.substring(2);

      } else {
        output = output.concat(template.substring(0, 1));
